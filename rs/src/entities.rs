...
//! HTML5 named character references, generated from the canonical table
//! in `ts/src/entities.ts` (itself generated from the WHATWG
//! entities.json). CommonMark 0.31.2 section 6.2 recognises only the
//! semicolon-terminated forms, so the semicolon-less legacy aliases the
//! WHATWG table also carries are absent: 2125 entries, not 2231. Names are
//! stored without the leading `&` and trailing `;`, sorted by byte value
//! so [`lookup`] can binary-search them.
//!
//! `tests/entities_test.rs` reads the TypeScript table and asserts this
//! one matches it entry for entry, so the two cannot drift.

/// The named references, as `(name, replacement)` pairs sorted by name.
...
/// The replacement for a named reference (name without `&` and `;`), or
/// `None` when section 6.2 admits no such name.
