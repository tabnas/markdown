...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/markdown.go
	rm -f go/markdown.go.bak
	git add go/markdown.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# `--all-targets` excludes doctests, and README.md is included as
# doctests from rs/src/lib.rs, so `--doc` runs separately.
test-rs:
