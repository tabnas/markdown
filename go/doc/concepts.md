...
The package parses CommonMark, with one GFM extension: strikethrough. That is the whole of
it. Tables, task list items, autolink literals, footnotes and disallowed-raw-HTML filtering
are not implemented, and the `GFM` option gates strikethrough and nothing else, because
there is nothing else to gate.

This is worth saying flatly because the previous documentation implied more. The honest
statement of scope is narrower than "CommonMark/GFM" and considerably more useful, since a
caller who needs tables now knows to look elsewhere rather than discovering it at runtime.
